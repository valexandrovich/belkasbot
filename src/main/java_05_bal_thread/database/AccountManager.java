...
public class AccountManager {
    ...
    public static synchronized boolean createSession(User user, long chatID){
        int updatedRows = 0;
        try{
    ...
    public static synchronized boolean setUserState(int userID, int state){
        int updatedRows = 0;
        try {
    ...
    public static synchronized boolean setUserLanguage(int userID, String languageCode){
        int updatedRows = 0;
        try {
            ...
            updatedRows = preparedStatement.executeUpdate();
            if (updatedRows>0) {LoggerService.logAction(LOGTAG,  getUserName(userID)
                    +" : changing user languageCode to " + languageCode);}
        } catch (SQLException e) {
            LoggerService.logError(LOGTAG, new Throwable(getUserName(userID) + " : " + e.getMessage()));
        }
        return updatedRows>0;
    }
    ...
    public static synchronized boolean setLastCallbackMessageID(int userID, long messageID){
        int updatedRows = 0;
            try {
                ...
                LoggerService.logAction(LOGTAG, getUserName(userID)
                        + " : setting lastCallbackMessageID = " + messageID);
            } catch (SQLException e) {
                LoggerService.logError(LOGTAG, new Throwable(getUserName(userID) + " : " + e.getMessage()));
            }
            return updatedRows > 0;
